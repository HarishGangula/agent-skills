...
**Don't reach for `!important` to win style conflicts.** It's an escape hatch that overrides the normal cascade, and once one rule uses it, the only way to beat that rule is *another* `!important` — so it escalates into an arms race that makes the stylesheet progressively unmaintainable. Instead, **manage specificity deliberately** so the right rule wins by the cascade's normal rules.

### How specificity works (the lever to pull instead)
Specificity is counted as (inline, IDs, classes/attributes/pseudo-classes, elements). Higher wins; ties go to source order. So to make a rule win, you don't need `!important` — you need slightly higher (but still low) specificity, or later source order, or better structure:
...
- **Use `@layer`** (modern, ideal): cascade layers let you define override order explicitly without any specificity inflation. Component layer beats base layer regardless of selector strength.
...
- Overriding a third-party widget that itself ships `!important` and exposes no API.
...
The test: is `!important` here a *documented, contained override strategy*, or is it patching a specificity mistake? The former is fine; the latter is the blocker.
...
- *"I'll use an ID, that's not !important."* IDs cause the same escalation problem one tier down; prefer classes and layers.
...
- ID selectors (`#header .nav`) used for styling rather than for JS hooks.
- Long descending chains (`.a .b .c .d .e {}`) that exist only to beat another rule.

## Verification

- Grep for `!important`. For each, classify: documented third-party/utility override (keep, note it) vs. specificity patch (fix).
- For a patch, open devtools, find the rule currently winning, and beat it by adding a class, scoping to a parent, reordering, or moving into a cascade layer.
- Confirm the removal didn't change rendering and that the new winning rule has the *lowest* specificity that still works.
