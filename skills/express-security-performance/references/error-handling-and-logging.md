...
**Handle errors centrally, never leak internals to clients, log safely.** Two intertwined concerns — one security, one stability:

1. **Don't leak stack traces or internals.** In production, an unhandled error must not send the stack trace, SQL error text, or file paths to the client — that's reconnaissance for an attacker and noise for a user. Use a central error-handling middleware that logs detail server-side and returns a generic message.
...
Any Express app — error handling and logging are universal. Pay special attention to `async` route handlers, `try/catch` scattered through handlers, or `console.log` in production code.
...
- *"`console.log` is fine."* Unstructured, unleveled, synchronous (can block), and easy to leave a secret in. Use a logger.
...
- No central `(err, req, res, next)` error middleware, or registered *before* the routes.
- `process.on('uncaughtException', () => { /* keep going */ })`.
- `console.log(req.body)` / logging tokens, passwords, or PII.

## Verification

- Trigger an error in production mode and confirm client gets a generic message, not a stack trace, while server log has full detail.
- Confirm error-handling middleware registered **last**, after all routes.
- For Express 4, confirm every async handler uses a wrapper or calls `next(err)` on failure.
- Grep logs/log calls for secrets, tokens, passwords, full bodies; confirm a structured logger over `console`.
